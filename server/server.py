...
from api.bdd.handler.ImageHandler import AvatarHandler
...
app.add_url_rule('/api/images/avatar',
                 view_func=AvatarHandler.as_view('avatar'), methods=['GET', 'POST'])

app.add_url_rule(
    '/api/users', view_func=UsersHandler.as_view('users'), methods=['GET', 'POST'])
app.add_url_rule('/api/user/<id>',
                 view_func=UserHandler.as_view('user'), methods=['GET', 'PATCH'])
app.add_url_rule(
